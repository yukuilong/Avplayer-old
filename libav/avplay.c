...
#ifndef ANDROID
			av_free(audio_frame.data[0]);  //android free it self
#endif
		}
	}
	return NULL;
		...
		if (ret != -1)
		{
			...
			if (!inited && play->m_vo_ctx)
			{
				...
				play->m_vo_ctx->fps = (float)play->m_base_info->video_frame_rate.num
					 / (float)play->m_base_info->video_frame_rate.den;
#ifdef ANDROID
				play->m_play_status = playing;
#else
...
#ifdef ANDROID
				__android_log_print(ANDROID_LOG_INFO,"video_render_thrd","%7.3f A-V: %7.3f A: %7.3f V: %7.3f FR: %d/fps, VB: %d/kbps\r",
			...
			// 释放视频帧缓冲. 
#ifndef ANDROID   //android free itslef
...
int logger(const char *fmt, ...)
{
	...
	// 输出到屏幕.
#ifdef ANDROID
	__android_log_print(ANDROID_LOG_INFO,"log","[%s] %s", time_buf, buffer);
#else
